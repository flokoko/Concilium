...
def run_pipeline(
    ...
    # Nur im LLM-Modus: holt den realisierten Return der jüngsten
    # Entscheidungs-Zeile für diesen Ticker und generiert einen
    # Reflexions-Block, der in Trader-/PM-Prompts injiziert wird.
    reflection_context = ""
    if llm is not None:
    ...
    if ensemble:
        ...
        trade = ensemble_trader(analysts, debate_result, llm, runs=ensemble_runs, feedback_context=feedback_context, reflection_context=reflection_context)
    else:
        logger.info("Schritt 4: Trader erstellt Trade-Vorschlag (Single-Run)")
        trade = trader(analysts, debate_result, llm, feedback_context=feedback_context, reflection_context=reflection_context)
    result["trade"] = trade
    ...
    final = portfolio_manager(trade, risk, llm, portfolio_fit=result.get("portfolio_fit"), feedback_context=feedback_context, reflection_context=reflection_context)
    result["final"] = final
